...
def extract(log=print):
    ...
    inc = [os.path.join(HERE, "shim"), paths.firmware_dir(), GEN, paths.tree_path("wled00/src/dependencies/fastled_slim")]
    probe_obj = os.path.join(paths.BUILD, "obj", "_probe_fx1d.o")
    for it in range(24):
        text, reg = _emit(top, items, skip, skip_pre, already)
        open(out, "w", encoding="utf-8").write(text)
