...
def build(app):
    ...
    with dpg.window(tag=TAG, label="MIDI controller", show=False, width=600, height=440, no_collapse=True):
        dpg.add_text("", tag="midi_note", color=c.DIM, wrap=580)
        with dpg.group(horizontal=True):
            dpg.add_text("PORT", color=c.ACCENT)
            dpg.add_combo([], tag="midi_port", width=300, callback=lambda s, v: open_port(app, v))
        ...
        dpg.add_text("MAPPINGS", color=c.ACCENT)
    ...
    for k, m in enumerate(st["maps"]):
        ...
        with dpg.group(horizontal=True, parent="midi_rows"):
            ...
            if t.get("kind") == "pin" and not t.get("bool"):
                for key in ("lo", "hi"):
                    dpg.add_input_float(width=80, step=0, format="%g", default_value=float(t.get(key, 0.0)), user_data=(k, key),
