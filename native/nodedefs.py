...
{ static uint16_t owed_ = 0; static float acc_ = 0.0f;
  ...
  $out.phase = acc_; }""",
    ...
    _n("Shape part", "coords", "pixel", [("pick", F, 0.0)], [("part", F), ("along", F), ("count", F), ("mask", B)], [],
       "$out.part = (float)part; $out.along = along; $out.count = (float)nparts; $out.mask = (part == (int)$in.pick);",
       "which part of the shape this pixel is in (a shape's parts in wiring order), where along it, how many parts; mask is true on the part picked"),
...
# the classic 5x7 font: five column bytes a glyph, bit 0 the top row, ASCII 32..126
FONT5X7 = [
...
def text_glyphs(text):
    """The font columns of a string, five bytes a character (unknown ones blank)."""
    out = []
    for ch in text:
        k = ord(ch) - 32
        out += FONT5X7[k * 5:k * 5 + 5] if 0 <= k < 95 else [0] * 5
    return out
